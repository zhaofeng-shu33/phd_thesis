...
的结论。
我们称 $X$ 和 $Y$ 是 \textbf{弱独立}的，如果 对于任何 $x \in \mathcal{X}$
$Y$关于$X$的条件 PMF 
...
\begin{example}
考虑 $P_0=(X_0,Y_0,Z_0)$是一个三维的随机向量，
其中$X_0,Y_0,Z_0$
是独立同分布的伯努利随机变量，每个随机变量从
$\{0, 1\}$之间取值。
定义 $P(x,y,z)=\frac{1}{8}(1+\epsilon (-1)^{x+y+z})$，
则 $P\in N^{(\epsilon)}(P_0)$，而
$\phi(x,y,z) = \frac{(-1)^{x+y+z}}{2\sqrt{2}}$。
