...
\begin{remark}
  ...
  给出的结论和 Hellinger 距离有关。
  对于两个$\R^d$中的向量$u,v$，它们的 Hellinger 距离
  的平方定义为
  \begin{equation}
    H^2(u||v) = \frac{1}{2}
    \sum_{i=1}^d (\sqrt{u}_i - \sqrt{v}_i)^2
  \end{equation}
  定义向量 $P_i= \frac{b\mathbf{1}_k + (a-b)e_k}{k}$。
  则对于$i\neq j$，
  $\sqrt{a} - \sqrt{b} > \sqrt{k}$
  可改写成
  $H(P_i||P_j)  = \frac{\sqrt{a} - \sqrt{b}}{\sqrt{k}} > 1$。
\end{remark}
