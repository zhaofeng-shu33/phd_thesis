...
\subsection{SIBM 模型}\label{sec:sibm_model}
...
令 $X_i$ 为i.i.d. 的随机变量的序列，且$X_1$ 存在 \gls{log_mgf}
$\Lambda(t)=\log \E[\exp(t X_1)]$ (log-MGF)。
...
另外，定理\ref{thm:cramer}中出现的$\Lambda^*(s)$和$\Lambda(s)$互为凸共轭函数：
\begin{definition}
...
\subsection{小节}
以上介绍的三种度量表面看起来关系不大，但它们都可以看做KL散度的某种推广。