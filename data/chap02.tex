...
\begin{theorem}
  ...
  \begin{enumerate}
    \item 若 $\gamma > b, m \geq m^*$，SIBM 可精确恢复；
    \item 若 $\gamma >b, m < m^*$，当$n\to \infty$ 时，所有SIBM 恢复算法的成功率趋近于零；
    \item 若 $ \gamma < b\beta, n = O(\log^{1/4}(n))$，SIBM无法精确恢复，特别地，对于$m$是不随$n$
    增长的常数时 SIBM 无法精确恢复；
    \item 若 $m=1, \beta > \beta^*$，则 $P(\sigma^{(1)}=\pm X) = 1-o(1)$
    \item 若 $m=1, \beta \leq \beta^*$，则
    $P(\Dist(\sigma^{(1)}, \pm X) =
