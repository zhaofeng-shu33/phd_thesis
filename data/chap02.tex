...
\gls{sbm}（SBM）是社群发现问题中最常用的统计模型之一
...
每个节点的标签为 $X_i$，从 $W= \{1, \omega, \dots, \omega^{k-1}\}$中取值。
为方便后续讨论，这里我们给$W$赋予群的结构，让它成为一个阶为$k$的循环群，
即$\omega^k=1$。
$W^n$ 表示 $W$的$n$次笛卡尔积。 

下面给出有$k$个社群的对称的随机块模型\footnote{
英文全称为 Symmetric Stochastic Block Model，
缩写为 SSBM}
的定义, 
...
\end{equation}
在微观层面上，概率越大或能量越低的状态越有可能出现；在宏观层面则表现为
磁化强度$M$的变化。
...
$\frac{\log n}{n}$ 主要是为了与其研究场景 $p,q=O(\log n / n)$
相对应。
...
类似式\ref{eq:hamiltonian}，标准玻茨模型的汉密尔顿能量公式为：
\begin{equation}
...
被叫做 估计量 $\hat{X}$ 的
准确概率。
令 $P_e(\hat{X}) = 1 - P_a(\hat{X})$
表示错误概率。
定义\ref{def:SSBMR} 也可以写成当$n\to \infty$
时
$P_e(\hat{X}) \to 0$。
记号 $\hat{X} \in S_k(X)$ 表示
我们只能获得相对于真实标签$X$的置换意义下的恢复结果。
因为没有一个类别的基准，
这种情况在无监督学习领域比较常见。
此外，我们要指出的是, 对给定的图 $G$, 估计量$\hat{X}$可以是确定性的也
可以是随机的。
...
将\ref{sec:sbm} 节 介绍的 SBM 模型与 \ref{sec:ising} 节介绍的 Ising 模型
复合起来可得到 \gls{sibm}（\gls{acr:sibm}）。
