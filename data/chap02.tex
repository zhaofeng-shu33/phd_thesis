...
将现实世界的网络建模成\gls{graph}(\glsdesc{graph})结构，社团内部连边
紧密、不同社团间连边稀疏。\gls{cd}(\glsdesc{cd})，也称社团检测，
即是在图结构上寻找特定的社团结构。
%社团根据其属性
...
\section{基于图强度的图分割算法}
根据\citet{fortunato2010community} 对社团发现算法的分类，图分割可用于社团发现，该类别属于20世纪提出的传统算法。
...
图强度概念的推广包括最小化平均误差\cite{mac}和多变量互信息等，其计算方法具有相似性。
图强度可以定义在无权图或有权图上，
...
$h(\lambda)$ 有多项式时间的算法，其中用到了\gls{psp}(\gls{acr:psp})的结构，
我们把这一算法称为PSP算法。
由于原文献年代较为久远，作为替代，下面我们参考\citet{mac}中对PSP算法
的描述对该算法进行进一步介绍。
\subsection{主分割序列}    
...
在统计物理学中，伊辛模型指的是
可以处于两种状态的自旋磁子的集合\cite{ising1925beitrag}。如图
...
雷尼散度在刻画随机块模型模型及其变体（如度修正的随机块模型）的误差衰减
