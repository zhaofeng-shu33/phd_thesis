...
在统计物理学中，伊辛模型是对铁磁性物质中的磁偶极矩建立的数学模型\cite{ising1925beitrag}。
...
$xx^{\mathrm{T}}$ 与 $A$ 的内积。令$\Theta=xx^{\mathrm{T}}$，则$Y$是一个
半正定的矩阵，若对$\Theta$秩为1的条件进行松弛，则
得到如下半正定规划问题：
\begin{align}\label{eq:opt_A_Y_1}
  \max_\Theta\  & \langle A, \Theta \rangle  \notag \\
  \textrm{s.t. } & \Theta_{ii} = 1, i \in [n] \notag \\
  & \langle \Theta, J_n \rangle  = 0
\end{align}
...
则
优化的目标函数还可写成 $ \langle R, \Theta \rangle $，
其中 $R$ 可进一步写成
...
\begin{theorem}
  设 $\hat{\Theta}_n$ 是
  优化问题 \eqref{eq:opt_A_Y_1} 的解，$x^*$ 是节点的真实标签向量。
  若 $\sqrt{a} - \sqrt{b}
  > \sqrt{2}$，
  则
  $\lim_{n\to\infty} P(\hat{\Theta}_n=x^*x^{*\mathrm{T}})=1$。
\end{theorem}
\section{信息论度量}
由于本文要用到多种信息论度量研究
...
称集合$\mathcal{C}(Z_V)$的元素为一个簇(Cluster)。
...
泛化能力\cite{huang2019information}等问题。
