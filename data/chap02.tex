...
难于计算。\citet{chan2017pin} 又提出了一种特殊的互独立网络 (\gls{acr:pin}) 模型，
...
\begin{align}
  ...
  \label{eq:czv}\\
  \textrm{其中 } C_{\gamma}(Z_V) &:= \textrm{maximal}
...
\end{equation}
\end{definition}
则我们称 $P$ 在 $P_0$的 $\epsilon$ 邻域 $N^{(\epsilon)}(P_0)$ 内，记作$P \in N^{(\epsilon)}(P_0)$。
...
其各字符在该序列中出现的比例，用符号$\mathbb{P}_x$表示，
而所有的$\mathbb{P}_x$组成集合$\mathbb{P}_n$。比如考虑字母集
...
$Q^n(E) = \sum_{x: \mathbb{P}_x \in E \cap \mathbb{P}_n} Q^n(x)$。
这里 $\mathbb{P}_x$ 是 $x$的型。
