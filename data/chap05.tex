...
定理 \ref{thm:Pe} \highlight{指出了对于有辅助信息的两社团随机块模型，
...
\highlight{此外，我们还针对 Lawyer friendship network 这一实际网络对
算法进行初步的验证。该网络描述了71间律师之间的朋友关系，对每一位律师附加有
年龄、性别等7种属性。}仿照Yuan等\cite{yuan2016community}的方法，
我们取其中的地位这一属性信息作为真实标签，利用其他6种属性信息，我们
分别通过 KMeans 方法（或谱嵌入）的方法得到分类标签（或一维特征）$\ell$，
并用于构造式\eqref{eq:B_lambda_def}中的辅助信息向量$h=\xi\cdot \ell$，
其中$\xi$是用来表示属性信息相对于图权重的超参数。
通过改变$\xi$的值，利用迭代方法求解得到不同的NMI值，
结果如图 \ref{fig:attribute_sdp} 所示。对于该数据集，我们发现对KMeans
获得的特征，较大的$\xi$可以提升社团检测性能；而对于谱嵌入得到的一维特征，
较小的$\xi$可以提升社团检测性能。
这一点可从如下的角度进行解释。
直接用 Kmeans 对属性聚类的NMI值为0.44，用谱嵌入得到的一维特征取中位数
的方法得到的NMI为0.34，而如果直接对图数据用聚类NMI值为 0.40。
因此，Kmeans得到的特征质量比图数据高，在SDP算法的输入变量构造中应赋予更大的权重。
与之相反，谱嵌入得到的一维特征质量比图数据低，相应的辅助信息权重较小可以取得更优的检测较果。
无论哪一种情形，结合图数据和辅助信息均可取得超过单一数据源的检测效果。
...
类数据满足这种条件。\highlight{但从理想化模型得到的结论仍具有对解决实际问题一般性的指导价值。
