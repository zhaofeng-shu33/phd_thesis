...
可得：
$$
I_+ = D(\hat{P}_{\lambda^*} || \Pois\left(\frac{a}{2}\right))
+ D(\hat{P}_{1-\lambda^*} || \Pois\left(\frac{b}{2}\right))
...
\begin{align}
    \max_{v}\  & h^{\mathrm{T}} v + \frac{1}{4}v^{\mathrm{T}} B v \notag \\
...
$(n+1)$ 维的向量。
	...
	\begin{equation}\label{eq:B_lambda_def}
		\widetilde{B} = \begin{pmatrix} 0 & h^{\mathrm{T}}  \\ h  & \frac{1}{2}B \end{pmatrix} 
	\end{equation}
