...
\begin{abstract}
  ...
  首先，基于多变量互信息这一信息论度量，
  我们建立了对弱独立的随机变量进行聚类与计算图上的主分割序列问题的联系。
  进一步地，我们利用多变量互信息优化了计算图上的主分割序列的算法，
  其时间复杂度相比现有计算主分割序列的方法降低了一个数量级。
 ...
 为算法超参的选取提供了理论解释。
  ...
  概率模型对其开展深入研究。在理论层面上我们导出了达到精确恢复所需的样本数量和该模型的最优误差衰减速率，
  而在算法设计层面上我们基于半正定规划设计了可以实现精确恢复的高效算法，
  并通过仿真实验验证了算法的有效性。

  总体而言，本文使用了多变量互信息、速率函数以及雷尼散度等信息论度量从理论层面上揭示了
  极限情况下社团发现算法可达的最优恢复率，保证了算法的检测效果。
  并籍此指导算法设计和进一步优化算法性能，节约计算资源，对于实际网络中的社团发现问题也有
 重要参考价值和借鉴意义。
