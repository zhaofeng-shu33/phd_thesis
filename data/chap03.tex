...
定义 \ref{def:weak_indepedent} 的拓展，\highlight{从而将弱独立的概念从两变量
推广到多变量。}
...
在式\eqref{eq:PXV}中，$\norm{B_{ij}}_{\mathrm{F}}^2 =O(\epsilon^2)$，
如果忽略高阶小量 $o(\epsilon^2)$，\highlight{因此定理} \ref{thm:DPX} 建立了多变量互信息和图强度的
...
前两个数据集如前所述是
人工生成的，
\highlight{而后三个数据集原是聚类问题中的标准数据集，
...
\highlight{由于 Glass 和 Ionosphere 两个数据集中异常值点数量较多，
而异常值点数量的上升使得HPSP发现的社团层次结构反映异常值点信息的难度增大，
从而导致信息检测的准确率下降。
因此，信息检测在这两个数据集上性能有待提升。}
\begin{table}
...
较理想的效果。最后，\highlight{由于权重的累积效应，基于HPSP算法获得的社团层次结构呈现边缘节点向中心节点汇聚的特点}，
