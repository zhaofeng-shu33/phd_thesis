...
并且类似 式 \eqref{eq:IP} 我们定义图的入割函数 (in-cut function) $f(C)$ for $C\subseteq V$ as $f(C) = \sum_{i\not\in C, j\in C, (i,j) \in E} w_{ij}$，
它是所有进入$C$的有向边的权值之和。
基于上面定义的符号，我们有
...
和 BHCD (贝叶斯层次聚类算法\cite{blundell2013bhcd})。
...
每个方法的超参数和相似度度量都经过了调优，TNR的最佳结果如表\ref{tab:odm}所示。我们可以看到，
...
其次，我们提出的社团层次发现的检测效果对网络中边的权值的选取较为敏感，在实际应用中需要调整赋权的参数才能达到
较理想的效果。最后，基于HPSP算法获得的社团层次结构呈现“滚雪球”的特点，
适用于有核心社团场景下的层次结构发现，而对平行结构下的层次结构检测能力较弱。

在本章的研究中我们在高斯块与具有两层结构的随机图上对HPSP算法
