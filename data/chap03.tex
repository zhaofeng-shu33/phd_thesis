...
为描述这种层次结构，在本节中，我们首次引入聚类树的概念，
然后利用聚类树的结构改进PSP算法，并在一定
的假设下证明该算法的理论复杂度。
最后，我们通过实验比较求解主划分序列的三种不同的算法的效率。需要指出的是，
我们的改进算法HPSP是在不改变PSP计算结果的前提下
降低算法的时间和空间复杂度。
\subsection{聚类树}\label{subsection:clustering_tree}
本章讨论的社团的概念的数学定义如下：
\begin{definition}
  ...
  \end{align}
\end{definition}
定义虽是用图强度定义了社团，但根据上一节的讨论，对于$\epsilon$-弱独立的随
机变量$Z_1,\dots, Z_n$且$I(Z_i;Z_j)=w_{ij}\epsilon^2$，
式 \eqref{eq:czv} 中的定义的 $\mathcal{C}(Z_V)$ 当$\epsilon\to 0$时  与$ \mathcal{C}(V)$ 等价，
因此多变量互信息的性质可用到讨论社团结构中来。

$\mathcal{C}(V)$中的集合存在相互包含关系，构成了社团的层次结构。
该层次结构可用聚类树来描述，
聚类树有一个根节点$V$，
其所有的叶节点为 $\{j\}$，其他节点均对应着
一个簇。
针对式\eqref{eq:czv}定义的簇，Chan等\cite{chan2020agglomerative} 提到了有两种生成
的方法，其相互等价，
但其描述过于复杂。
本小节引入的聚类树可以更为方便直观地
对这两种生成方式进行描述。
...
聚类树可以由PSP算法先计算出主划分序列，再由主划分序列转换而来，
...
快 $5$ 倍。
\subsection{聚类树的数据结构}
为提升算法效率， \ref{subsection:clustering_tree} 节引入的聚类树在算法实现中需要使用高效的数据结构进行表示。
...
另外，HPSP算法也可用于计算主划分序列，此时需要从构造好的聚类树中获取正确的划分。
