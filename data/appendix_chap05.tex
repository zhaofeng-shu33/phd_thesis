\section{定理证明}\label{sec:sbmsi_proof}
...
$$
\max_{x \in \{\pm\}^n} \sum_{i=1}^n \sum_{j=1}^m x_i \log \frac{P_0(y_i^{(j)})}{P_1(y_i^{(j)})}
...
则我们只需考虑
$$
\max_{x \in \{\pm\}^n}\  \frac{1}{\log a/b}\sum_{i=1}^n \sum_{j=1}^m x_i \log \frac{P_0(y_i^{(j)})}{P_1(y_i^{(j)})}
...
则上式等价于：
$$
\max_{x \in \{\pm\}^n}\  \frac{1}{\log a/b}\sum_{i=1}^n \sum_{j=1}^m v_i \log \frac{P_0(y_i^{(j)})}{P_1(y_i^{(j)})}
...
首先直接将$(1,g)(1,g)^{\mathrm{T}}$代入验证可知条件 (a) 满足。
...
$$
对于最后一项 $(g^{\perp})^{\mathrm{T}} \Xi_n g^{\perp}$，
