...
因为$G$中各边的存在与否是独立的，
对于 $s\neq 0$
我们有 $A^s_r \sim \textrm{Binomial}(\frac{n}{k}, \B ) $，
而 $A^0_r \sim \textrm{Binomial}(\frac{n}{k}-1, \A )$。
...
\begin{lemma}\label{lem:minus}
  ...
  个坐标位置上不同于真值标签向量
  $X$。
	对于 $i\neq j$，令 $I_{ij} = |\{r\in [n] | X_r = w^i, \sigma_r = w^j \}$
  且 $I_{ii} = 0$。
  我们进一步将行和表示为 $I_i = \sum_{j=0}^{k-1} I_{ij}$ 并且
	列和为 $I'_i = \sum_{j=0}^{k-1} I_{ji}$。
	则
\begin{align}
...
\begin{proof}
  ...
  \beta(1 + \frac{\gamma \log n}{n}) \sum_{\bar{\sigma}_i = \bar{\sigma}_j} Z_{ij}
	- \beta\frac{\gamma \log n}{n} \sum_{\bar{\sigma}_i = \bar{\sigma}_j} 1  - \log Z_G(\gamma, \beta)
  \end{align*}
