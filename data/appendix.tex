...
\begin{lemma}\label{lem:SBM_tr_counting_cross}
	Consider $\SSBM(2n, 2, p, q)$ and count the number of triangles $T$, which has a node in $S_1$ and an edge in $S_2$.
...
\begin{lemma}\label{lem:SBM_tr_counting_3}
	Consider $\SSBM(3n, 3, p, q)$ and count the number of triangles $T$, which has a node in $S_1$, one node in $S_2$, and one node in $S_3$.
	Then the variance of $T$ is:
...
\begin{lemma}\label{lem:sbmV}
	For a $\SSBM(n, k, p, q)$ where $p=\frac{a\log n}{n}, q = \frac{b\log n}{n}$. The~number of triangles is $T$.
